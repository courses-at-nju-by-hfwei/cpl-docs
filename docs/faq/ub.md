...
在FAQ的2-if-for-array对全局变量(`global variable`)和局部变量(`local variable`)的[简要介绍](faq/2-if-for-array)中，已经说明了局部变量不会被系统自动初始化的问题，因而未被初始化的局部变量中存储的是不确定的垃圾值，具体的说是上次使用这块内存后剩下的值，如果让垃圾值参与程序，程序的正确性就是不确定的，例如：
